...
with open(sys.argv[3]) as f:
    for line in f:
        ...
        chrn = temp[0]
        pos = int(temp[1])
        if not chrn in snp_seek_snpsAF:
            snp_seek_snpsAF[chrn] = dict()
        snp_seek_snpsAF[chrn][pos] = '.'
...
with open(sys.argv[1]) as f:
    for line in f:
        ...
        info_str = temp[7]
        if snp_flag:
            if chrn in snp_seek_snpsAF:
                if pos in snp_seek_snpsAF[chrn]:
                    info_str = info_str.rstrip(';') + ';SNPSeekAF=' + str(snp_seek_snpsAF[chrn][pos])
